#!/bin/bash
 ...
 clear
 msg -bar
 rm -rf /etc/Vcyrus/demo-ssh 2>/dev/null
 mkdir /etc/Vcyrus/demo-ssh 2>/dev/null
 SCPdir="/etc/Vcyrus"
 ...
 rm -rf /tmp/cmd
 touch /etc/Vcyrus/demo-ssh/$name
 echo "pass: $pass" >> /etc/Vcyrus/demo-ssh/$name
 echo "data: ($tmp)Minutes" >> /etc/Vcyrus/demo-ssh/$name
 msg -bar2
 echo -e "\033[1;93m ¡¡ TEMPORARY USER x MINUTES (Vcyrus By @CyrusAGN) !!\033[0m"
 msg -bar2
