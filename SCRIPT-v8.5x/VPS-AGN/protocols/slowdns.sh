#!/bin/bash
 #by @rufu99
 ADM_inst="/etc/Vcyrus/Slow/install" && [[ ! -d ${ADM_inst} ]] && exit
 ADM_slow="/etc/Vcyrus/Slow/Key" && [[ ! -d ${ADM_slow} ]] && exit
 info(){
 	clear
 	nodata(){
 		msg -bar
 ...
 ini_slow(){
     ...
     if [[ ! -e ${ADM_inst}/dns-server ]]; then
     	...
     	if wget -O ${ADM_inst}/dns-server https://raw.githubusercontent.com/Cyrusagn/Vcyrus_English_Official/master/LINKS-LIBRARIES/dns-server &>/dev/null ; then
