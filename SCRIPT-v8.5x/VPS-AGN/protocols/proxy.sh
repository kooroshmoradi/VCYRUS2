#!/bin/bash
 ...
 det_py(){
 	...
 	rm /etc/Vcyrus/PortPD.log &>/dev/null
 	rm -rf /etc/Vcyrus/protocols/lacasitamx.py
 ...
 sleep 1.s
 [[ ! -e /etc/Vcyrus/fix ]] && touch /etc/Vcyrus/fix
 else
 echo ""
 fi
 clear
 ...
 tput cuu1 && tput dl1
     ...
     echo -e "$BARRA"
     [[ ! -e /etc/Vcyrus/protocols/lacasitamx.py ]] && rm -rf /etc/Vcyrus/protocols/lacasitamx.py
 #
 	# Initializing or Proxy
 	(
 	less << CPM > /etc/Vcyrus/protocols/lacasitamx.py
 # -*- coding: utf-8 -*-
...
CPM
 ...
 chmod +x /etc/Vcyrus/protocols/lacasitamx.py &>/dev/nulll
 
 screen -dmS pydic-"$proxycolor" python ${SCPinst}/lacasitamx.py "$proxycolor" "$mensage" && echo ""$proxycolor" "$mensage"" >> /etc/Vcyrus/PortPD.log
 }
