...
USRdatabase="/etc/Vcyrus/RegV2ray"
...
rm -rf /etc/Vcyrus/RegV2ray > /dev/null 2>&1
...
##Registro
echo "  $UUID | $nick | $valid " >> /etc/Vcyrus/RegV2ray
Fecha=`date +%d-%m-%y-%R`
cp /etc/Vcyrus/RegV2ray /etc/Vcyrus/v2ray/RegV2ray-"$Fecha"
cp /etc/v2ray/config.json /etc/Vcyrus/v2ray/config.json-"$Fecha" 
v2ray restart > /dev/null 2>&1
echo ""
v2ray info > /etc/Vcyrus/v2ray/confuuid.log
lineP=$(sed -n '/'${UUID}'/=' /etc/Vcyrus/v2ray/confuuid.log)
numl1=4
let suma=$lineP+$numl1
sed -n ${suma}p /etc/Vcyrus/v2ray/confuuid.log 
echo ""
msg -bar
...
echo -e "\e[33m$(cat /etc/Vcyrus/RegV2ray|cut -d '|' -f2,1)" 
msg -bar
...
linePre=$(sed -n '/'${uuidel}'/=' /etc/Vcyrus/RegV2ray)
sed -i "${linePre}d" /etc/Vcyrus/RegV2ray
numl1=2
...
msg -bar
# usersss=$(cat /etc/Vcyrus/RegV2ray|cut -d '|' -f1)
# cat /etc/Vcyrus/RegV2ray|cut -d'|' -f3
VPSsec=$(date +%s)
local HOST="/etc/Vcyrus/RegV2ray"
local HOST2="/etc/Vcyrus/RegV2ray"
...
DateExp="$(cat /etc/Vcyrus/RegV2ray|grep -w "$hostreturn"|cut -d'|' -f3)"
if [[ ! -z $DateExp ]]; then             
...
usris="$(cat /etc/Vcyrus/RegV2ray|grep -w "$hostreturn"|cut -d'|' -f2)"
...
linesss=$(cat /etc/Vcyrus/RegV2ray | wc -l)
...
estarts () {
VPSsec=$(date +%s)
local HOST="/etc/Vcyrus/v2ray/lisportt.log"
local HOST2="/etc/Vcyrus/v2ray/lisportt.log"
local RETURN="$(cat $HOST|cut -d'|' -f2)"
local IDEUUID="$(cat $HOST|cut -d'|' -f1)"
if [[ -z $RETURN ]]; then
...
i=1
while read hostreturn ; do
iptables -n -v -L > /etc/Vcyrus/v2ray/data1.log 
statsss=$(cat /etc/Vcyrus/v2ray/data1.log|grep -w "tcp spt:$hostreturn quota:"|cut -d' ' -f3,4,5)
gblim=$(cat /etc/Vcyrus/v2ray/lisportt.log|grep -w "$hostreturn"|cut -d'|' -f2)
...
[[ ! -z $contador_secuencial ]] && {
linesss=$(cat /etc/Vcyrus/v2ray/lisportt.log | wc -l)
...
echo ""
echo " $portbg | $gbuser | $multiplicacion " >> /etc/Vcyrus/v2ray/lisportt.log 
msg -bar
msg -ne "Enter To Continue" && read enter
${SCPinst}/v2ray.sh
}
...
[[ $(sed -n '/'${portbg}'/=' /etc/Vcyrus/v2ray/lisportt.log|head -1) ]] || invaliduuid
gblim=$(cat /etc/Vcyrus/v2ray/lisportt.log|grep -w "$portbg"|cut -d'|' -f3)
sudo iptables -D OUTPUT -p tcp --sport $portbg -j DROP
sudo iptables -D OUTPUT -p tcp --sport $portbg -m quota --quota $gblim -j ACCEPT
iptables-save > /etc/iptables/rules.v4
lineP=$(sed -n '/'${portbg}'/=' /etc/Vcyrus/v2ray/lisportt.log)
sed -i "${linePre}d" /etc/Vcyrus/v2ray/lisportt.log
msg -bar
msg -ne "Enter To Continue" && read enter
${SCPinst}/v2ray.sh 
}
