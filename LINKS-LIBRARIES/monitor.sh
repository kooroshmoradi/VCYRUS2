...
IDB=`less /etc/Vcyrus/controller/IDT.log` > /dev/null 2>&1
...
<head>
  ...
  <title>Vcyrus Service Monitor</title>
  <link rel='stylesheet' href='estilos.css'>
</head>
<body>
<h1>Monitor Service By @CyrusAGN</h1>
...
ureset_python () {
for port in $(cat /etc/Vcyrus/PortPD.log| grep -v "nobody" |cut -d' ' -f1)
do
PIDVRF3="$(ps aux|grep pydic-"$port" |grep -v grep|awk '{print $2}')"
if [[ -z $PIDVRF3 ]]; then
screen -dmS pydic-"$port" python /etc/Vcyrus/protocolos/PDirect.py "$port"
NOM=`less /etc/Vcyrus/controller/nombre.log` > /dev/null 2>&1
NOM1=`echo $NOM` > /dev/null 2>&1
IDB=`less /etc/Vcyrus/controller/IDT.log` > /dev/null 2>&1
IDB1=`echo $IDB` > /dev/null 2>&1
...
#SERVICE PY+SSL
ureset_pyssl () {
for port in $(cat /etc/Vcyrus/PySSL.log| grep -v "nobody" |cut -d' ' -f1)
do
PIDVRF3="$(ps aux|grep pyssl-"$port" |grep -v grep|awk '{print $2}')"
if [[ -z $PIDVRF3 ]]; then
screen -dmS pyssl-"$port" python /etc/Vcyrus/protocolos/python.py "$port"
NOM=`less /etc/Vcyrus/controller/nombre.log` > /dev/null 2>&1
NOM1=`echo $NOM` > /dev/null 2>&1
IDB=`less /etc/Vcyrus/controller/IDT.log` > /dev/null 2>&1
IDB1=`echo $IDB` > /dev/null 2>&1
