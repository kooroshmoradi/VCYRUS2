...
v1=$(curl -sSL "https://raw.githubusercontent.com/Cyrusagn/Vcyrus_English_Official/master/SCRIPT-v8.5x/Version")
...
repo() {
  link="https://raw.githubusercontent.com/Cyrusagn/Multi-Script/main/Source-List/20.04.list"
  case $1 in
...
post_reboot() {
  echo 'wget -O /root/install.sh "https://raw.githubusercontent.com/Cyrusagn/Vcyrus_English_Official/master/installer/install-without-key.sh"; clear; sleep 2; chmod +x /root/install.sh; /root/install.sh --continue' >>/root/.bashrc
...
clear && clear
msg -bar2
echo -e " \e[5m\033[1;100m   =====>> ►► 🐲 Vcyrus - SCRIPT  🐲 ◄◄ <<=====   \033[1;37m"
msg -bar2
print_center -ama "AVAILABLE SCRIPT LIST"
msg -bar
#-BASH SOPORTE ONLINE
wget https://raw.githubusercontent.com/Cyrusagn/Vcyrus_English_Official/master/LINKS-LIBRARIES/SPR.sh -O /usr/bin/SPR >/dev/null 2>&1
chmod +x /usr/bin/SPR
...
install_official() {
  ...
  msg -bar
  clear && clear
  mkdir /etc/Vcyrus >/dev/null 2>&1
  cd /etc
  wget https://raw.githubusercontent.com/Cyrusagn/Vcyrus_English_Official/master/SCRIPT-v8.5x/Vcyrus.tar.xz >/dev/null 2>&1
  tar -xf Vcyrus.tar.xz >/dev/null 2>&1
  chmod +x Vcyrus.tar.xz >/dev/null 2>&1
  rm -rf Vcyrus.tar.xz
  cd
  chmod -R 755 /etc/Vcyrus
  rm -rf /etc/Vcyrus/MEUIPvps
  echo "/etc/Vcyrus/menu" >/usr/bin/menu && chmod +x /usr/bin/menu
  echo "/etc/Vcyrus/menu" >/usr/bin/VPSAGN && chmod +x /usr/bin/VPSAGN
  wget https://raw.githubusercontent.com/Cyrusagn/Vcyrus_English_Official/master/LINKS-LIBRARIES/monitor.sh -P /bin/
  echo "$slogan" >/etc/Vcyrus/message.txt
  ...
  [[ ! -d /etc/Vcyrus/B-Vcyrususer ]] && mkdir /etc/Vcyrus/B-Vcyrususer
  ...
  cd
  [[ ! -d /etc/Vcyrus/v2ray ]] && mkdir /etc/Vcyrus/v2ray
  [[ ! -d /etc/Vcyrus/Slow ]] && mkdir /etc/Vcyrus/Slow
  [[ ! -d /etc/Vcyrus/Slow/install ]] && mkdir /etc/Vcyrus/Slow/install
  [[ ! -d /etc/Vcyrus/Slow/Key ]] && mkdir /etc/Vcyrus/Slow/Key
  touch /usr/share/lognull &>/dev/null
  wget -O /bin/resetsshdrop https://raw.githubusercontent.com/Cyrusagn/Vcyrus_English_Official/master/LINKS-LIBRARIES/resetsshdrop &>/dev/null
  ...
  v1=$(curl -sSL "https://raw.githubusercontent.com/Cyrusagn/Vcyrus_English_Official/master/SCRIPT-v8.5x/Version")
  echo "$v1" >/etc/versin_script
  wget -O /etc/versin_script_new https://raw.githubusercontent.com/Cyrusagn/Vcyrus_English_Official/master/SCRIPT-v8.5x/Version &>/dev/null
  ...
  echo 'wget -O /etc/versin_script_new https://raw.githubusercontent.com/Cyrusagn/Vcyrus_English_Official/master/SCRIPT-v8.5x/Version &>/dev/null' >>.bashrc
  echo 'echo "" ' >>.bashrc
  echo 'mess1="$(less /etc/Vcyrus/message.txt)" ' >>.bashrc
  echo 'echo "" ' >>.bashrc
